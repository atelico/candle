...
use ebcandle::{IndexOp, Layout, Result, Shape, Tensor, D};
use ebcandle_nn::{linear, Linear, VarBuilder};
...
impl EuclideanCodebook {
    ...
    pub fn decode(&self, indexes: &Tensor) -> Result<Tensor> {
        ...
        // let ys = ebcandle_nn::Embedding::new(self.embedding.clone(), self.dim).forward(xs)?;
...
impl ResidualVectorQuantization {
    ...
    pub fn decode(&self, xs: &Tensor) -> Result<Tensor> {
        if self.layers.is_empty() {
            ebcandle::bail!("empty layers in ResidualVectorQuantization")
        }
...
pub struct ResidualVectorQuantizer {
    ...
    input_proj: Option<ebcandle_nn::Conv1d>,
    output_proj: Option<ebcandle_nn::Conv1d>,
}
...
impl ResidualVectorQuantizer {
    pub fn new(
        ...
        let input_proj = if input_dim == dim && !force_projection {
            ...
            let c = ebcandle_nn::conv1d_no_bias(
                input_dim,
                dim,
                1,
                Default::default(),
