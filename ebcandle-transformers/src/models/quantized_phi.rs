...
use ebcandle_nn::{Embedding, LayerNorm};
...
impl QLinear {
    fn new<R: std::io::Read + std::io::Seek>(
        ...
        let inner = ebcandle::quantized::QMatMul::from_qtensor(w)?;
...
impl LayerWeights {
    fn apply_rotary_emb(&self, xs: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        let xs_rot = ebcandle_nn::rotary_emb::rope(&xs_rot.contiguous()?, &cos, &sin)?;
