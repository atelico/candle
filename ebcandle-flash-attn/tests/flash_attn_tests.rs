use anyhow::Result;
use ebcandle::{DType, Device, IndexOp, Tensor, D};
...
fn flash_attn_acausal() -> Result<()> {
    ...
    let ys2 = {
        ...
        ebcandle_flash_attn::flash_attn(&q, &k, &v, 0.5, false)?.transpose(1, 2)?
    };
...
fn flash_attn_varlen() -> Result<()> {
    ...
    let ys = {
        ...
        ebcandle_flash_attn::flash_attn_varlen(
...
fn flash_attn_acausal_softcap() -> Result<()> {
    ...
    let ys2 = {
        ...
        let k = k.transpose(1, 2)?;
        let v = v.transpose(1, 2)?;
        ebcandle_flash_attn::flash_attn_softcap(&q, &k, &v, 0.5, Some(30.), false)?.transpose(1, 2)?
    };
    let ys2 = ys2.i(0)?.to_dtype(DType::F32)?;
    let diff = ys1.sub(&ys2)?.abs()?.flatten_all()?.max(0)?;
...
fn flash_attn_varlen_softcap() -> Result<()> {
    ...
    let ys = {
        ...
        let k = k.transpose(0, 1)?;
        let v = v.transpose(0, 1)?;
        ebcandle_flash_attn::flash_attn_varlen_softcap(
            &q,
            &k,
            &v,
            &seqlens_q,
