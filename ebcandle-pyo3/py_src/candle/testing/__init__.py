...
_UNSIGNED_DTYPES = set([str(ebcandle.u8), str(ebcandle.u32)])
...
def assert_almost_equal(
    ...
    if str(actual.dtype) in _UNSIGNED_DTYPES or str(expected.dtype) in _UNSIGNED_DTYPES:
        actual = actual.to(ebcandle.i64)
        expected = expected.to(ebcandle.i64)

    diff = (actual - expected).abs()

    threshold = (expected.abs().to_dtype(ebcandle.f32) * rtol + atol).to(expected)
