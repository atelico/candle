...
# extern crate ebcandle_hf_hub;
use ebcandle_hf_hub::api::sync::Api;
use ebcandle_core::Device;
...
let weights = ebcandle_core::safetensors::load(weights, &Device::Cpu);
```
...
# 
# let weights = repo.get("model.safetensors").unwrap();
use ebcandle_core::{Device, Tensor, DType};
use ebcandle_nn::{Linear, Module};

let weights = ebcandle_core::safetensors::load(weights, &Device::Cpu).unwrap();
...
For a full reference, you can check out the full [bert](https://github.com/LaurentMazare/ebcandle/tree/main/ebcandle-examples/examples/bert) example.
