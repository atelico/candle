...
use ebcandle_nn::{Embedding, Module, VarBuilder};
...
impl Embeddings {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let word_embeddings =
            ebcandle_nn::embedding(config.vocab_size, config.dim, vb.pp("word_embeddings"))?;
        let position_embeddings = ebcandle_nn::embedding(
...
impl MultiHeadSelfAttention {
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let weights = ebcandle_nn::ops::softmax(&scores, ebcandle::D::Minus1)?;
