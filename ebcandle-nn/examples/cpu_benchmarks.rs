...
use ebcandle::quantized::GgmlType;
use ebcandle::{CpuStorage, Device, Layout, Module, Result, Shape, Tensor, D};
...
impl ebcandle::CustomOp1 for Im2Col {
...
impl Benchmark for QMatMul {
    type PreProcessData = (ebcandle::quantized::QMatMul, Tensor);
    type RunResult = Tensor;
    fn preprocess() -> Result<Self::PreProcessData> {
        let zeros = vec![ebcandle::quantized::k_quants::BlockQ4_0::zeros(); 4096 * 11008 / 32];
        let mm = ebcandle::quantized::QTensor::new(
            ebcandle::quantized::QStorage::Cpu(Box::new(zeros)),
            (4096, 11008),
        )?;
        let mm = ebcandle::quantized::QMatMul::from_qtensor(mm)?;
...
impl Benchmark for Softmax {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        ebcandle_nn::ops::softmax(d, D::Minus1)
    }
...
impl Benchmark for SoftmaxLastDim {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        ebcandle_nn::ops::softmax_last_dim(d)
    }
