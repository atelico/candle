...
use ebcandle::{DType, Result, Tensor};
use ebcandle_nn::{
...
pub struct TrOCRConfig {
    ...
    pub activation_function: ebcandle_nn::Activation,
...
impl Default for TrOCRConfig {
    fn default() -> Self {
        Self {
            ...
            activation_function: ebcandle_nn::Activation::Gelu,
...
impl TrOCRAttention {
    ...
    fn forward(
        ...
        let attn_probs = ebcandle_nn::ops::softmax_last_dim(&attn_weights)?;
...
impl TrOCRForCausalLM {
    pub fn new(decoder_cfg: &TrOCRConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let output_projection = if decoder_cfg.tie_word_embeddings {
            ebcandle_nn::Linear::new(decoder.embed_tokens.embeddings().clone(), None)
        } else {
            ebcandle_nn::linear_no_bias(
                decoder_cfg.d_model,
