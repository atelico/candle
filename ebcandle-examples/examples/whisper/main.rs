...
use ebcandle::{Device, IndexOp, Tensor};
use ebcandle_nn::{ops::softmax, VarBuilder};
...
use ebcandle_transformers::models::whisper::{self as m, audio, Config};
...
impl Model {
    ...
    pub fn encoder_forward(&mut self, x: &Tensor, flush: bool) -> ebcandle::Result<Tensor> {
        match self {
    ...
    pub fn decoder_final_linear(&self, x: &Tensor) -> ebcandle::Result<Tensor> {
        match self {
...
impl Decoder {
    ...
    fn decode(&mut self, mel: &Tensor, t: f64) -> Result<DecodingResult> {
        ...
        for i in 0..sample_len {
            ...
            let prob = softmax(&logits, ebcandle::D::Minus1)?
...
pub fn token_id(tokenizer: &Tokenizer, token: &str) -> ebcandle::Result<u32> {
    match tokenizer.token_to_id(token) {
        None => ebcandle::bail!("no token-id for {token}"),
        Some(id) => Ok(id),
    }
}
...
struct Args {
    ...
    /// repo: https://huggingface.co/datasets/Narsil/ebcandle_demo/
    #[arg(long)]
    input: Option<String>,
...
fn main() -> Result<()> {
    ...
    let (config_filename, tokenizer_filename, weights_filename, input) = {
        let api = Api::new()?;
        let dataset = api.dataset("Narsil/ebcandle-examples".to_string());
        ...
        let sample = if let Some(input) = args.input {
            ...
            println!("No audio file submitted: Downloading https://huggingface.co/datasets/Narsil/ebcandle_demo/blob/main/samples_jfk.wav");
