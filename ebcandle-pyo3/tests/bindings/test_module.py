import ebcandle
from ebcandle import Tensor, QTensor
from ebcandle.nn import Module, Linear
...
def test_module_can_load_statedict():
    ...
    statedict = {
        "linear.weight": ebcandle.ones((20, 10)),
        "linear.bias": ebcandle.zeros((20,)),
        "t": Tensor(42.0),
    }
    a = A()
...
def test_module_can_load_quantized_tensors():
    class A(Module):
        def __init__(self):
            super().__init__()
            self.t = ebcandle.randn((16, 256))
            self._quantizable_buffers.add("t")

    statedict = {
        "t": ebcandle.ones((16, 256)).quantize("q4_0"),
    }
    a = A()
    a.load_state_dict(statedict)
