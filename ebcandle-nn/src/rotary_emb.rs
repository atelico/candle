use ebcandle::{CpuStorage, Layout, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
impl ebcandle::CustomOp3 for RotaryEmbI {
    ...
    fn cpu_fwd(
        ...
        fn inner<T: ebcandle::WithDType + num_traits::Float>(
            src: &[T],
            l_src: &Layout,
            cos: &[T],
            l_cos: &Layout,
            ...
            let src = match l_src.contiguous_offsets() {
                None => ebcandle::bail!("input src has to be contiguous"),
                Some((o1, o2)) => &src[o1..o2],
            };
            let cos = match l_cos.contiguous_offsets() {
                None => ebcandle::bail!("input cos has to be contiguous"),
                Some((o1, o2)) => &cos[o1..o2],
            };
            let sin = match l_sin.contiguous_offsets() {
                None => ebcandle::bail!("input sin has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        use ebcandle::cuda_backend::{kernel_name, kernels, WrapErr};
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            let src = match l_src.contiguous_offsets() {
                None => ebcandle::bail!("src input has to be contiguous"),
                Some((o1, o2)) => src.slice(o1..o2),
            };
            let cos = match l_cos.contiguous_offsets() {
                None => ebcandle::bail!("cos input has to be contiguous"),
                Some((o1, o2)) => cos.slice(o1..o2),
            };
            let sin = match l_sin.contiguous_offsets() {
                None => ebcandle::bail!("sin input has to be contiguous"),
        ...
        use ebcandle::backend::BackendStorage;
        use ebcandle::cuda_backend::CudaStorageSlice::{BF16, F16, F32, F64};
        let dev = s1.device();
    ...
    fn metal_fwd(
        &self,
        src: &ebcandle::MetalStorage,
        l_src: &Layout,
        cos: &ebcandle::MetalStorage,
        l_cos: &Layout,
        sin: &ebcandle::MetalStorage,
        l_sin: &Layout,
        ...
        let name = match src.dtype() {
            ebcandle::DType::F32 => "rope_i_f32",
            ebcandle::DType::F16 => "rope_i_f16",
            ebcandle::DType::BF16 => "rope_i_bf16",
            dtype => ebcandle::bail!("rope-i is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let el = b * h * t * d;
        let output = device.new_buffer(el, src.dtype(), "rope-i")?;
        ebcandle_metal_kernels::call_rope_i(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        let out = ebcandle::MetalStorage::new(output, device.clone(), el, src.dtype());
...
pub fn rope_i(xs: &Tensor, cos: &Tensor, sin: &Tensor) -> Result<Tensor> {
    ...
    if !xs.is_contiguous() {
        ebcandle::bail!("xs has to be contiguous in rope")
    }
    if !cos.is_contiguous() {
        ebcandle::bail!("cos has to be contiguous in rope")
    }
    if !sin.is_contiguous() {
        ebcandle::bail!("sin has to be contiguous in rope")
    }
...
impl ebcandle::CustomOp3 for RotaryEmb {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            ebcandle::DType::F32 => "rope_f32",
            ebcandle::DType::F16 => "rope_f16",
            ebcandle::DType::BF16 => "rope_bf16",
            dtype => ebcandle::bail!("rope is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let el = b * h * t * d;
        let output = device.new_buffer(el, src.dtype(), "rope-i")?;
        ebcandle_metal_kernels::call_rope(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
...
impl ebcandle::CustomOp3 for RotaryEmbThd {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            ebcandle::DType::F32 => "rope_thd_f32",
            ebcandle::DType::F16 => "rope_thd_f16",
            ebcandle::DType::BF16 => "rope_thd_bf16",
            dtype => ebcandle::bail!("rope_thd is not implemented for {dtype:?}"),
        };
        let (b, t, h, d) = l_src.shape().dims4()?;
        let el = b * h * t * d;
        let output = device.new_buffer(el, src.dtype(), "rope-thd")?;
        ebcandle_metal_kernels::call_rope_thd(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
