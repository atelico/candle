...
class Linear(Module):
    ...
    def __init__(
        ...
        self.weight = ebcandle.ones((out_features, in_features), **factory_kwargs)
        if bias:
            self.bias = ebcandle.zeros((out_features,), **factory_kwargs)
        else:
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if isinstance(self.weight, ebcandle.QTensor):
            if len(dims) < 3:
