use ebcandle_core::backend::BackendStorage;
use ebcandle_core::cpu_backend;
use ebcandle_core::test_utils::to_vec1_round;
use ebcandle_core::{CpuStorage, CustomOp1, DType, Device, Error, Layout, Result, Shape, Tensor};
...
impl CustomOp1 for Elu {
    ...
    fn cpu_fwd(&self, s: &CpuStorage, l: &Layout) -> Result<(CpuStorage, Shape)> {
        let storage = ebcandle_core::map_dtype!(
            "elu",
            s,
...
fn custom_op1_with_backward() -> Result<()> {
    let cpu = &Device::Cpu;
    let t = ebcandle_core::Var::new(&[-2f32, 0f32, 2f32], cpu)?;
...
impl ebcandle_core::InplaceOp1 for Elu {
    fn name(&self) -> &'static str {
        "elu"
    }
    ...
    fn cpu_fwd(&self, s: &mut CpuStorage, _l: &Layout) -> Result<()> {
        ...
        match s {
            ...
            _ => ebcandle_core::bail!("unsupported dtype for inplace elu"),
        }
        Ok(())
    }
}
...
fn ug_op() -> Result<()> {
    ...
    let device = if ebcandle_core::utils::cuda_is_available() {
        Device::new_cuda(0)?
    } else if ebcandle_core::utils::metal_is_available() {
        Device::new_metal(0)?
    } else {
        ebcandle_core::bail!("metal/cuda is mandatory for this test")
    };
    let op = ebcandle_core::UgIOp1::new("test", kernel, &device)?;
