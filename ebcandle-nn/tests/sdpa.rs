...
mod metal_sdpa_tests {
    ...
    fn sdpa_full() -> ebcandle::Result<()> {
        ...
        let ground_truth = {
            ...
            let att = ebcandle_nn::ops::softmax_last_dim(&att.to_dtype(DType::F32)?)?
                .to_dtype(q.dtype())?;
            att.matmul(&v.clone())?
        };

        let sdpa_output = ebcandle_nn::ops::sdpa(&q, &k, &v, scale as f32, 1.)?;
    ...
    #[test]
    fn sdpa_vector() -> ebcandle::Result<()> {
        use ebcandle::{DType, Device, Tensor};
    ...
    #[test]
    fn sdpa_full_softcapping() -> ebcandle::Result<()> {
        use ebcandle::{DType, Device, Tensor};
        use std::ops::{Div, Mul};
        ...
        let ground_truth = {
            let att = (q.clone() * scale)?.matmul(&k.clone().t()?)?;
            let att = ebcandle_nn::ops::softmax_last_dim(
        ...
        let sdpa_output = ebcandle_nn::ops::sdpa(&q, &k, &v, scale as f32, SOFTCAP as f32)?;
    ...
    #[test]
    fn sdpa_vector_softcapping() -> ebcandle::Result<()> {
        use ebcandle::{DType, Device, Tensor};
        use std::ops::{Div, Mul};
    ...
    #[test]
    fn sdpa_vector_cross() -> ebcandle::Result<()> {
        use ebcandle::{DType, Device, Tensor};
