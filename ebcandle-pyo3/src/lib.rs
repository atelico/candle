...
use ::ebcandle::{quantized::QTensor, DType, Device, Module, Tensor, WithDType};
...
fn actual_index(t: &Tensor, dim: usize, index: i64) -> ::ebcandle::Result<usize> {
    let dim = t.dim(dim)?;
    if 0 <= index {
        let index = index as usize;
        if dim <= index {
            ::ebcandle::bail!("index {index} is too large for tensor dimension {dim}")
        }
        Ok(index)
    } else {
        if (dim as i64) < -index {
            ::ebcandle::bail!("index {index} is too low for tensor dimension {dim}")
        }
        Ok((dim as i64 + index) as usize)
    }
}

fn actual_dim(t: &Tensor, dim: i64) -> ::ebcandle::Result<usize> {
    let rank = t.rank();
    if 0 <= dim {
        let dim = dim as usize;
        if rank <= dim {
            ::ebcandle::bail!("dimension index {dim} is too large for tensor rank {rank}")
        }
        Ok(dim)
    } else {
        if (rank as i64) < -dim {
            ::ebcandle::bail!("dimension index {dim} is too low for tensor rank {rank}")
        }
        Ok((rank as i64 + dim) as usize)
    }
}

// TODO: Something similar to this should probably be a part of ebcandle core.
trait MapDType {
    type Output;
...
impl PyTensor {
    ...
    /// Converts ebcandle's tensor to pytorch's tensor
    /// &RETURNS&: torch.Tensor
    fn to_torch(&self, py: Python<'_>) -> PyResult<PyObject> {
        let ebcandle_values = self.values(py)?;
        let torch_tensor: PyObject = py
            .import_bound("torch")?
            .getattr("tensor")?
            .call1((ebcandle_values,))?
            .extract()?;
        Ok(torch_tensor)
    }
    ...
    fn __richcmp__(&self, rhs: &Bound<PyAny>, op: CompareOp) -> PyResult<Self> {
        ...
        if let Ok(rhs) = rhs.extract::<PyTensor>() {
            if self.0.shape() == rhs.0.shape() {
                ...
                // We broadcast manually here because `ebcandle.cmp` does not support automatic broadcasting
...
impl PyQTensor {
    ...
    fn matmul_t(&self, lhs: &PyTensor) -> PyResult<PyTensor> {
        let qmatmul = ::ebcandle::quantized::QMatMul::from_arc(self.0.clone()).map_err(wrap_err)?;
...
fn load_safetensors(path: &str, py: Python<'_>) -> PyResult<PyObject> {
    let res = ::ebcandle::safetensors::load(path, &Device::Cpu).map_err(wrap_err)?;
    let res = res
        .into_iter()
...
fn save_safetensors(
    ...
    ::ebcandle::safetensors::save(&tensors, path).map_err(wrap_err)
}
...
fn load_ggml(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    let ggml =
        ::ebcandle::quantized::ggml_file::Content::read(&mut file, &device).map_err(wrap_err)?;
    let tensors = ggml
        .tensors
        .into_iter()
        .map(|(key, qtensor)| Ok((key, PyQTensor(Arc::new(qtensor)).into_py(py))))
        .collect::<::ebcandle::Result<Vec<_>>>()
        .map_err(wrap_err)?;
...
fn load_gguf(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    use ::ebcandle::quantized::gguf_file;
...
fn cuda_is_available() -> bool {
    ::ebcandle::utils::cuda_is_available()
}

#[pyfunction]
/// Returns true if ebcandle was compiled with 'accelerate' support.
/// &RETURNS&: bool
fn has_accelerate() -> bool {
    ::ebcandle::utils::has_accelerate()
}

#[pyfunction]
/// Returns true if ebcandle was compiled with MKL support.
/// &RETURNS&: bool
fn has_mkl() -> bool {
    ::ebcandle::utils::has_mkl()
}

#[pyfunction]
/// Returns the number of threads used by the ebcandle.
/// &RETURNS&: int
fn get_num_threads() -> usize {
    ::ebcandle::utils::get_num_threads()
}

fn ebcandle_utils(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
fn softmax(tensor: PyTensor, dim: i64) -> PyResult<PyTensor> {
    ...
    let sm = ebcandle_nn::ops::softmax(&tensor.0, dim).map_err(wrap_err)?;
    Ok(PyTensor(sm))
}
...
fn silu(tensor: PyTensor) -> PyResult<PyTensor> {
    let s = ebcandle_nn::ops::silu(&tensor.0).map_err(wrap_err)?;
    Ok(PyTensor(s))
}
...
fn ebcandle_functional_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[cfg(feature = "onnx")]
fn ebcandle_onnx_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[pymodule]
fn ebcandle(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    let utils = PyModule::new_bound(py, "utils")?;
    ebcandle_utils(py, &utils)?;
    m.add_submodule(&utils)?;
    let nn = PyModule::new_bound(py, "functional")?;
    ebcandle_functional_m(py, &nn)?;
    m.add_submodule(&nn)?;
    #[cfg(feature = "onnx")]
    {
        let onnx = PyModule::new_bound(py, "onnx")?;
        ebcandle_onnx_m(py, &onnx)?;
