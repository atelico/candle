...
impl CausalSelfAttention {
    ...
    fn forward(&self, x: &Tensor, index_pos: usize, block_idx: usize) -> Result<Tensor> {
        ...
        let att = ebcandle_nn::ops::softmax(&att, D::Minus1)?;
...
impl Mlp {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let x = (ebcandle_nn::ops::silu(&self.c_fc1.forward(x)?)? * self.c_fc2.forward(x)?)?;
...
impl Block {
    ...
    fn load(vb: VarBuilder, cache: &Cache, cfg: &Config) -> Result<Self> {
        ...
        let input_layernorm =
            ebcandle_nn::rms_norm_non_quant(cfg.dim, cfg.norm_eps, vb.pp("input_layernorm"))?;
        let post_attention_layernorm = ebcandle_nn::rms_norm_non_quant(
            cfg.dim,
            cfg.norm_eps,
...
impl Llama {
    ...
    pub fn load(vb: VarBuilder, cache: &Cache, cfg: &Config) -> Result<Self> {
        ...
        let norm = ebcandle_nn::rms_norm_non_quant(cfg.dim, cfg.norm_eps, vb.pp("model.norm"))?;
