...
use ebcandle::{DType, Device, IndexOp, Module, Result, Tensor, D};
use ebcandle_nn::{linear_b as linear, Linear, VarBuilder};
use std::sync::Arc;
...
pub struct Config {
    ...
    pub hidden_activation: ebcandle_nn::Activation,
...
impl RotaryEmbedding {
    pub(crate) fn new(dtype: DType, cfg: &Config, dev: &Device) -> Result<Self> {
        if cfg.partial_rotary_factor != 0.5 {
            ebcandle::bail!("partial-rotary-factor {} <> 0.5", cfg.partial_rotary_factor)
        }
...
impl Rglru {
    ...
    pub(crate) fn forward(&mut self, xs: &Tensor, pos: usize) -> Result<Tensor> {
        ...
        let input_gate = ebcandle_nn::ops::sigmoid(&input_gate)?;
        let res = baddbmm(
        ...
        let recurrent_gate = ebcandle_nn::ops::sigmoid(&recurrent_gate)?;
...
impl RecurrentBlock {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let conv_1d = ebcandle_nn::conv1d(
            lru_width,
            lru_width,
            cfg.conv1d_width,
    ...
    pub fn forward(&mut self, xs: &Tensor, pos: usize) -> Result<Tensor> {
        ...
        let x_branch = if pos == 0 {
            ...
            let conv_state = match self.conv1d_state.as_ref() {
                None => ebcandle::bail!("empty cache despite pos > 0"),
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let embed_tokens =
            ebcandle_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("embed_tokens"))?;
