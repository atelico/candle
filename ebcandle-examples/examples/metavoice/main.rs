...
use ebcandle_transformers::models::encodec;
use ebcandle_transformers::models::metavoice::{adapters, gpt, tokenizers, transformer};
use ebcandle_transformers::models::quantized_metavoice::transformer as qtransformer;

use ebcandle::{DType, IndexOp, Tensor};
...
fn main() -> Result<()> {
    ...
    let api = Api::new()?;
    let repo = api.model("lmz/ebcandle-metavoice".to_string());
    ...
    let mut first_stage_model = if args.quantized {
        ...
        let vb =
            ebcandle_transformers::quantized_var_builder::VarBuilder::from_gguf(filename, &device)?;
    ...
    let spk_emb = ebcandle::safetensors::load(&spk_emb_file, &ebcandle::Device::Cpu)?;
    ...
    for logits in logits.iter() {
        ...
        for step in 0..seq_len {
            ...
            let prs = ebcandle_nn::ops::softmax_last_dim(logits)?.to_vec1::<f32>()?;
    ...
    let pcm = ebcandle_examples::audio::normalize_loudness(&pcm, 24_000, true)?;
    let pcm = pcm.to_vec1::<f32>()?;
    let mut output = std::fs::File::create(&args.out_file)?;
    ebcandle_examples::wav::write_pcm_as_wav(&mut output, &pcm, 24_000)?;
    Ok(())
}
