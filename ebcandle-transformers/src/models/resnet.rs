...
use ebcandle::{Result, D};
use ebcandle_nn::{batch_norm, Conv2d, Func, VarBuilder};
...
fn conv2d(
    ...
    ebcandle_nn::conv2d_no_bias(c_in, c_out, ksize, conv2d_cfg, vb)
}
...
fn resnet(
    ...
    let fc = match nclasses {
        None => None,
        Some(nclasses) => {
            let linear = ebcandle_nn::linear(512, nclasses, vb.pp("fc"))?;
            Some(linear)
        }
    };
    Ok(Func::new(move |xs| {
...
fn bottleneck_resnet(
    ...
    let fc = match nclasses {
        None => None,
        Some(nclasses) => {
            let linear = ebcandle_nn::linear(4 * 512, nclasses, vb.pp("fc"))?;
            Some(linear)
        }
    };
    Ok(Func::new(move |xs| {
