...
use ebcandle_nn::{Conv1d, Conv1dConfig, ConvTranspose1d, ConvTranspose1dConfig, VarBuilder};
...
impl ebcandle::Module for Snake1d {
...
impl ebcandle::Module for ResidualUnit {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let ys = xs
            .apply(&self.snake1)?
            .apply(&self.conv1)?
...
impl ebcandle::Module for EncoderBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.res1)?
            .apply(&self.res2)?
            .apply(&self.res3)?
...
impl ebcandle::Module for Encoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl ebcandle_nn::Module for DecoderBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl ebcandle::Module for Decoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let mut xs = xs.apply(&self.conv1)?;
        for block in self.blocks.iter() {
            xs = xs.apply(block)?
...
pub struct VectorQuantizer {
    in_proj: Conv1d,
    out_proj: Conv1d,
    codebook: ebcandle_nn::Embedding,
}
...
impl VectorQuantizer {
    pub fn new(in_dim: usize, cb_size: usize, cb_dim: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let codebook = ebcandle_nn::embedding(cb_size, cb_dim, vb.pp("codebook"))?;
        Ok(Self {
            in_proj,
            out_proj,
            codebook,
...
impl ResidualVectorQuantizer {
    ...
    pub fn from_codes(&self, codes: &Tensor) -> Result<Tensor> {
        ...
        match sum {
            Some(s) => Ok(s),
            None => ebcandle::bail!("empty codebooks"),
        }
    }
}
