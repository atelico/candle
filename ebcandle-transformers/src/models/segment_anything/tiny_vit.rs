...
use ebcandle::{IndexOp, Result, Tensor, D};
use ebcandle_nn::{Conv2dConfig, Module, VarBuilder};
...
#[derive(Debug)]
struct Conv2dBN {
    c: ebcandle_nn::Conv2d,
    bn: ebcandle_nn::BatchNorm,
    span: tracing::Span,
}

impl Conv2dBN {
    fn new(in_: usize, out: usize, ks: usize, cfg: Conv2dConfig, vb: VarBuilder) -> Result<Self> {
        let c = ebcandle_nn::conv2d_no_bias(in_, out, ks, cfg, vb.pp("c"))?;
        let bn = ebcandle_nn::batch_norm(out, 1e-5, vb.pp("bn"))?;
...
impl MBConv {
    fn new(in_: usize, out: usize, expand_ratio: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let cfg2 = ebcandle_nn::Conv2dConfig {
            padding: 1,
            groups: hidden,
            ..Default::default()
        };
...
impl Mlp {
    fn new(in_: usize, hidden: usize, vb: VarBuilder) -> Result<Self> {
        let norm = ebcandle_nn::layer_norm(in_, 1e-5, vb.pp("norm"))?;
...
impl Attention {
    fn new(
        ...
        let h = dh + nh_kd * 2;
        let norm = ebcandle_nn::layer_norm(dim, 1e-5, vb.pp("norm"))?;
...
pub struct TinyViT {
    ...
    // norm_head: ebcandle_nn::LayerNorm,
    // head: ebcandle_nn::Linear,
...
impl TinyViT {
    pub fn new(
        ...
        // let norm_head = ebcandle_nn::layer_norm(last_embed_dim, 1e-5, vb.pp("norm_head"))?;
        // let head = ebcandle_nn::linear(last_embed_dim, num_classes, vb.pp("head"))?;
        let neck_conv1 =
            ebcandle_nn::conv2d_no_bias(last_embed_dim, 256, 1, Default::default(), vb.pp("neck.0"))?;
        let neck_ln1 = super::LayerNorm2d::new(256, 1e-6, vb.pp("neck.1"))?;
        let cfg = ebcandle_nn::Conv2dConfig {
            padding: 1,
            ..Default::default()
        };
        let neck_conv2 = ebcandle_nn::conv2d_no_bias(256, 256, 3, cfg, vb.pp("neck.2"))?;
