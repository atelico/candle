...
use ebcandle::{bail, DType, Device, Result, Tensor};
...
impl Attr for str {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        std::str::from_utf8(&attr.s).map_err(ebcandle::Error::wrap)
    }
}
...
impl Attr for GraphProto {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        attr.g
            .as_ref()
            .ok_or_else(|| ebcandle::Error::Msg("attribute does not contain graph".to_string()))
    }
}
...
impl AttrOwned for Vec<String> {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<Self> {
        ...
        for bytes in attr.strings.iter() {
            let s = String::from_utf8(bytes.clone()).map_err(ebcandle::Error::wrap)?;
            ret.push(s);
        }
        Ok(ret)
    }
        ...
        match node.op_type.as_str() {
            ...
            "LogSoftmax" => {
                ...
                let output = match get_attr_opt::<i64>(node, "axis")? {
                    None => ebcandle_nn::ops::softmax_last_dim(input)?,
                    Some(&axis) => {
                        let axis = input.normalize_axis(axis)?;
                        ebcandle_nn::ops::log_softmax(input, axis)?
                    }
                };
                values.insert(node.output[0].clone(), output);
            }
            "Softmax" => {
                ...
                let output = match get_attr_opt::<i64>(node, "axis")? {
                    ...
                    Some(&axis) => {
                        let axis = input.normalize_axis(axis)?;
                        ebcandle_nn::ops::softmax(input, axis)?
                    }
                };
                values.insert(node.output[0].clone(), output);
            }
            ...
            "Gather" => {
                ...
                // tensor directly, but ebcandle does not support tensor indexing at the moment, so
            ...
            "Sigmoid" => {
                let input = get(&node.input[0])?;
                let output = ebcandle_nn::ops::sigmoid(input)?;
                values.insert(node.output[0].clone(), output);
            }
            "Gelu" => {
                let input = get(&node.input[0])?;
            ...
            "LeakyRelu" => {
                ...
                let output = ebcandle_nn::ops::leaky_relu(input, alpha.into())?;
                values.insert(node.output[0].clone(), output);
            }
            ...
            "LSTM" => {
                ...
                let vmap = ebcandle_nn::VarMap::new();
                vmap.data().lock().unwrap().extend([
                    ("weight_ih_l0".to_string(), ebcandle::Var::from_tensor(&w)?),
                    ("weight_hh_l0".to_string(), ebcandle::Var::from_tensor(&r)?),
                    ("bias_ih_l0".to_string(), ebcandle::Var::from_tensor(&wb)?),
                    ("bias_hh_l0".to_string(), ebcandle::Var::from_tensor(&rb)?),
                ]);
                use ebcandle_nn::rnn::RNN as _;
                let lstm = ebcandle_nn::rnn::lstm(
                    input_size,
                    hidden_size as usize,
                    ebcandle_nn::rnn::LSTMConfig::default(),
                    ebcandle_nn::VarBuilder::from_varmap(&vmap, w.dtype(), w.device()),
                )?;

                let mut lstm_state = ebcandle_nn::rnn::LSTMState::new(h, c);
