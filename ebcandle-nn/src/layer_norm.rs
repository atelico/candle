...
//!
//! ```rust
//! use ebcandle::{Tensor, Device::Cpu, test_utils::to_vec3_round};
//! use ebcandle_nn::{LayerNorm, Module};
//! # fn main() -> ebcandle::Result<()> {
//!
        ...
        match (
            ...
            (Storage::Cuda(x_storage), Storage::Cuda(weight_storage)) => {
                match (x_storage.dtype(), weight_storage.dtype()) {
                    ...
                    _ => ebcandle::bail!("DType mismatch in fused rmsnorm."),
                }
            }
            _ => unreachable!(),
        }
