...
use ebcandle::{Module, Result, StreamTensor, StreamingModule, Tensor, D};
use ebcandle_nn::{Conv1d, VarBuilder};
...
impl NormConv1d {
    ...
    pub fn new(
        ...
        norm: Option<Norm>,
        bias: bool,
        cfg: ebcandle_nn::Conv1dConfig,
        vb: VarBuilder,
    ) -> Result<Self> {
        let conv = match norm {
            None | Some(Norm::TimeGroupNorm) => {
                if bias {
                    ebcandle_nn::conv1d(in_c, out_c, k_size, cfg, vb.pp("conv"))?
                } else {
                    ebcandle_nn::conv1d_no_bias(in_c, out_c, k_size, cfg, vb.pp("conv"))?
                }
            }
            Some(Norm::WeightNorm) => {
                conv1d_weight_norm(in_c, out_c, k_size, bias, cfg, vb.pp("conv"))?
            }
            Some(Norm::SpectralNorm) => ebcandle::bail!("SpectralNorm is not supported yet."),
        };
        let norm = match norm {
            None | Some(Norm::WeightNorm) | Some(Norm::SpectralNorm) => None,
            Some(Norm::TimeGroupNorm) => {
                if causal {
                    ebcandle::bail!("GroupNorm doesn't support causal evaluation.")
                }
                let norm = ebcandle_nn::group_norm(1, out_c, 1e-5, vb.pp("norm"))?;
                Some(norm)
            }
        };
        Ok(Self {
...
fn unpad1d(xs: &Tensor, unpad_l: usize, unpad_r: usize) -> Result<Tensor> {
    ...
    if len < unpad_l + unpad_r {
        ebcandle::bail!("unpad1d: tensor len {len} is too low, {unpad_l} + {unpad_r}")
    }
...
impl StreamableConv1d {
    ...
    pub fn new(
        ...
        let cfg = ebcandle_nn::Conv1dConfig {
            padding: 0,
            stride,
            dilation,
            groups,
        ...
        if k_size < stride {
            ebcandle::bail!("kernel-size {k_size} is smaller than stride {stride}")
        }
        Ok(Self {
            conv,
            causal,
...
impl ConvDownsample1d {
    pub fn new(
        ...
        if !learnt {
            ebcandle::bail!("only learnt=true is supported")
        }
...
mod tests {
    ...
    fn run_conv1d(
        ...
        let dev = &ebcandle::Device::Cpu;
        let vm = ebcandle_nn::VarMap::new();
        let vb = VarBuilder::from_varmap(&vm, ebcandle::DType::F32, dev);
        ...
        if diff > 1e-5 {
            ...
            println!("{ys}");
            println!("{ys_steps}");
            ebcandle::bail!("larger diff than expected {diff}")
        }
        Ok(())
    }
