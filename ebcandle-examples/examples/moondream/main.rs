...
/// (3, 378, 378).
pub fn load_image<P: AsRef<std::path::Path>>(p: P) -> ebcandle::Result<Tensor> {
    ...
    (data.to_dtype(ebcandle::DType::F32)? / 255.)?
        .broadcast_sub(&mean)?
        .broadcast_div(&std)
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let (model_id, revision) = match args.model_id {
        ...
        None => {
            if args.quantized {
                ("santiagomed/ebcandle-moondream".to_string(), None)
            } else {
                (
