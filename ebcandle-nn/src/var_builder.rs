...
use crate::VarMap;
use ebcandle::{safetensors::Load, DType, Device, Error, Result, Shape, Tensor};
...
impl SimpleBackend for HashMap<String, Tensor> {
    fn get(
        ...
        let tensor = self
            ...
            .clone();
        if tensor.shape() != &s {
            Err(ebcandle::Error::UnexpectedShape {
...
impl SimpleBackend for ebcandle::npy::NpzTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for ebcandle::pickle::PthTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for ebcandle::safetensors::MmapedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for ebcandle::safetensors::BufferedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> SimpleBackend for ebcandle::safetensors::SliceSafetensors<'a> {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> VarBuilder<'a> {
    ...
    pub unsafe fn from_mmaped_safetensors<P: AsRef<std::path::Path>>(
        ...
        let tensors = ebcandle::safetensors::MmapedSafetensors::multi(paths)?;
    ...
    pub fn from_buffered_safetensors(data: Vec<u8>, dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = ebcandle::safetensors::BufferedSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_slice_safetensors(data: &'a [u8], dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = ebcandle::safetensors::SliceSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_npz<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let npz = ebcandle::npy::NpzTensors::new(p)?;
    ...
    pub fn from_pth<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let pth = ebcandle::pickle::PthTensors::new(p, None)?;
    ...
    ///
    /// ```rust
    /// use ebcandle::{Tensor, DType, Device};
    ///
    ...
    /// .into_iter()
    /// .collect();
    /// let vb = ebcandle_nn::VarBuilder::from_tensors(tensors, DType::F32, &Device::Cpu);
...
pub struct ShardedSafeTensors(ebcandle::safetensors::MmapedSafetensors);
...
impl Backend for ShardedSafeTensors {
    ...
    fn get(
        ...
        let iterator = if dim == 0 {
            ...
            ebcandle::bail!("Get sharded on dimensions != 0 or 1")
        };
