use ebcandle::quantized::{gguf_file, GgmlDType, QTensor};
use ebcandle::{Device, Result};
...
fn run_print(
    ...
    if full {
        ebcandle::display::set_print_options_full();
    }
    if let Some(line_width) = line_width {
        ebcandle::display::set_line_width(line_width)
    }
    ...
    match format {
        Format::Npz => {
            let tensors = ebcandle::npy::NpzTensors::new(file)?;
        ...
        Format::Safetensors => {
            use ebcandle::safetensors::Load;
            let tensors = unsafe { ebcandle::safetensors::MmapedSafetensors::new(file)? };
        ...
        Format::Pth => {
            let pth_file = ebcandle::pickle::PthTensors::new(file, None)?;
            let names = if names.is_empty() {
                pth_file
                    .tensor_infos()
                    .keys()
        ...
        Format::Pickle => {
            ebcandle::bail!("pickle format is not supported for print")
        }
        Format::Ggml => {
            let mut file = std::fs::File::open(file)?;
            let content = ebcandle::quantized::ggml_file::Content::read(&mut file, device)?;
            let names = if names.is_empty() {
    ...
    match format {
        ...
        Format::Safetensors => {
            ...
            for (name, view) in tensors.iter() {
                ...
                let dtype = match ebcandle::DType::try_from(dtype) {
        ...
        Format::Pth => {
            let mut tensors = ebcandle::pickle::read_pth_tensor_info(file, verbose, None)?;
        ...
        Format::Pickle => {
            ...
            let mut stack = ebcandle::pickle::Stack::empty();
...
fn run_quantize_safetensors(
    ...
    for in_file in in_files.iter() {
        let in_tensors = ebcandle::safetensors::load(in_file, &Device::Cpu)?;
...
fn run_dequantize(
    ...
    ebcandle::safetensors::save(&tensors, out_file)?;
    Ok(())
}
...
fn run_quantize(
    ...
    if in_files.is_empty() {
        ebcandle::bail!("no specified input files")
    }
    if let Some(extension) = out_file.extension() {
        if extension == "safetensors" {
            ebcandle::bail!("the generated file cannot use the safetensors extension")
        }
    }
    ...
    if in_files.len() != 1 {
        ebcandle::bail!("only a single in-file can be used when quantizing gguf files")
    }
