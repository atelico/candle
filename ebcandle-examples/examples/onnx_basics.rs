...
pub fn main() -> Result<()> {
    ...
    match args.command {
        Command::Print { file } => {
            let model = ebcandle_onnx::read_file(file)?;
            println!("{model:?}");
        ...
        Command::SimpleEval { file } => {
            ...
            for input in graph.input.iter() {
                ...
                let value = match type_ {
                    ebcandle_onnx::onnx::type_proto::Value::TensorType(tt) => {
                        let dt = match DataType::try_from(tt.elem_type) {
                            Ok(dt) => match ebcandle_onnx::dtype(dt) {
                                Some(dt) => dt,
                                None => {
                                    anyhow::bail!(
                        ...
                        let dims = shape
                                ...
                                .map(|dim| match dim.value.as_ref().expect("no dim value") {
                                    ebcandle_onnx::onnx::tensor_shape_proto::dimension::Value::DimValue(v) => Ok(*v as usize),
                                    ebcandle_onnx::onnx::tensor_shape_proto::dimension::Value::DimParam(_) => Ok(42),
                                })
            ...
            let outputs = ebcandle_onnx::simple_eval(&model, inputs)?;
