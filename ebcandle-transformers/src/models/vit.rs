...
use ebcandle_nn::{layer_norm, LayerNorm, VarBuilder};
...
impl Embeddings {
    ...
    pub fn forward(
        ...
        let embeddings = match (bool_masked_pos, &self.mask_token) {
            (None, _) => embeddings,
            (Some(_), None) => ebcandle::bail!("bool_masked_pos set without mask_token"),
...
struct Intermediate {
    dense: Linear,
    intermediate_act_fn: ebcandle_nn::Activation,
}
