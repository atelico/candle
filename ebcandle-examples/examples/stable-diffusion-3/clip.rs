...
use ebcandle::{DType, IndexOp, Module, Tensor, D};
use ebcandle_transformers::models::{stable_diffusion, t5};
...
impl T5WithTokenizer {
    fn new(vb: ebcandle_nn::VarBuilder, max_position_embeddings: usize) -> Result<Self> {
...
pub struct StableDiffusion3TripleClipWithTokenizer {
    ...
    clip_g_text_projection: ebcandle_nn::Linear,
    t5: T5WithTokenizer,
}
...
impl StableDiffusion3TripleClipWithTokenizer {
    pub fn new_split(
        ...
        let vb_clip_g = unsafe {
            ebcandle_nn::VarBuilder::from_mmaped_safetensors(&[clip_g_file], DType::F16, device)?
        };
        let vb_clip_l = unsafe {
            ebcandle_nn::VarBuilder::from_mmaped_safetensors(&[clip_l_file], DType::F16, device)?
        };
        let vb_t5 = unsafe {
            ebcandle_nn::VarBuilder::from_mmaped_safetensors(&[t5xxl_file], DType::F32, device)?
        };
        ...
        let text_projection =
            ebcandle_nn::linear_no_bias(1280, 1280, vb_clip_g.pp("text_projection"))?;
        ...
        // Also see:
        // https://github.com/huggingface/ebcandle/issues/2480
        // https://github.com/huggingface/ebcandle/pull/2481
    ...
    pub fn new(vb_fp16: ebcandle_nn::VarBuilder, vb_fp32: ebcandle_nn::VarBuilder) -> Result<Self> {
        let max_position_embeddings = 77usize;
        let clip_l = ClipWithTokenizer::new(
        ...
        let text_projection = ebcandle_nn::linear_no_bias(
            1280,
            1280,
