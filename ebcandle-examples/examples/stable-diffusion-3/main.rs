...
use ebcandle_transformers::models::mmdit::model::{Config as MMDiTConfig, MMDiT};
...
struct Args {
    ...
    #[arg(
        long,
        default_value = "A cute rusty robot holding a ebcandle torch in its hand, \
...
fn main() -> Result<()> {
    ...
    let (mmdit_config, mut triple, vb) = if which.is_3_5() {
        ...
        let vb = unsafe {
            ebcandle_nn::VarBuilder::from_mmaped_safetensors(&[model_file], DType::F16, &device)?
        };
        ...
        let vb_fp32 = unsafe {
            ebcandle_nn::VarBuilder::from_mmaped_safetensors(&[model_file], DType::F32, &device)?
        };
