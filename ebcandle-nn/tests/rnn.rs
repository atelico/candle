...
use ebcandle::{test_utils::to_vec2_round, DType, Device, Result, Tensor};
use ebcandle_nn::RNN;
...
fn lstm() -> Result<()> {
    ...
    .into_iter()
    .collect();
    let vb = ebcandle_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let lstm = ebcandle_nn::lstm(2, 3, Default::default(), vb)?;
...
fn gru() -> Result<()> {
    ...
    .collect();
    let vb = ebcandle_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let gru = ebcandle_nn::gru(2, 3, Default::default(), vb)?;
