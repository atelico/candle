...
use ebcandle_nn::{self as nn, layer_norm::RmsNormNonQuantized};
...
pub struct AttnProjections {
    head_dim: usize,
    qkv: nn::Linear,
    ln_k: Option<ebcandle_nn::RmsNorm<RmsNormNonQuantized>>,
    ln_q: Option<ebcandle_nn::RmsNorm<RmsNormNonQuantized>>,
    proj: nn::Linear,
}
...
impl AttnProjections {
    pub fn new(dim: usize, num_heads: usize, vb: nn::VarBuilder) -> Result<Self> {
        ...
        let (ln_k, ln_q) = if vb.contains_tensor("ln_k.weight") {
            let ln_k = ebcandle_nn::rms_norm_non_quant(head_dim, 1e-6, vb.pp("ln_k"))?;
            let ln_q = ebcandle_nn::rms_norm_non_quant(head_dim, 1e-6, vb.pp("ln_q"))?;
            (Some(ln_k), Some(ln_q))
        } else {
            (None, None)
        };
