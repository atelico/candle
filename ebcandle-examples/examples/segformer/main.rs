...
use ebcandle_transformers::models::segformer::{
...
struct SegmentationArgs {
    ...
    #[arg(
        ...
        default_value = "ebcandle-examples/examples/segformer/assets/labels.json"
    )]
    label_path: PathBuf,
...
fn get_vb_and_config(model_name: String, device: &Device) -> anyhow::Result<(VarBuilder, Config)> {
    ...
    let vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[model_file], ebcandle::DType::F32, device)? };
...
fn segmentation_task(args: SegmentationArgs, device: &Device) -> anyhow::Result<()> {
    ...
    let image = ebcandle_examples::imagenet::load_image224(args.image)?
        .unsqueeze(0)?
        .to_device(device)?;
...
fn classification_task(args: ClassificationArgs, device: &Device) -> anyhow::Result<()> {
    ...
    let classification = ebcandle_nn::ops::softmax_last_dim(&classification)?;
