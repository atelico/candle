use ebcandle::{test_device, Device, IndexOp, Result, Tensor};
use ebcandle_core as ebcandle;
...
fn strided_blocks() -> Result<()> {
    use ebcandle::Device::Cpu;
    let tensor = Tensor::arange(0u32, 24u32, &Cpu)?.reshape((2, 3, 4))?;
    match tensor.strided_blocks() {
        ebcandle::StridedBlocks::SingleBlock { start_offset, len } => {
            assert_eq!(start_offset, 0);
            assert_eq!(len, 24);
        }
        ebcandle::StridedBlocks::MultipleBlocks { .. } => {
    ...
    let tensor = tensor.i((.., 1))?;
    match tensor.strided_blocks() {
        ebcandle::StridedBlocks::SingleBlock { .. } => {
            panic!("unexpected block structure")
        }
        ebcandle::StridedBlocks::MultipleBlocks {
            block_len,
