...
use ebcandle::{safetensors::Load, DType, Device, IndexOp, Tensor, D};
use ebcandle_nn::{ops::softmax, VarBuilder};
pub use ebcandle_transformers::models::whisper::{self as m, Config};
...
impl Decoder {
    ...
    pub fn load(md: ModelData) -> anyhow::Result<Self> {
        ...
        let model = if md.quantized {
            let vb = ebcandle_transformers::quantized_var_builder::VarBuilder::from_gguf_buffer(
                &md.weights,
                &device,
            )?;
...
pub fn detect_language(model: &mut Model, tokenizer: &Tokenizer, mel: &Tensor) -> Result<u32, E> {
    ...
    let probs = ebcandle_nn::ops::softmax(&logits, D::Minus1)?;
