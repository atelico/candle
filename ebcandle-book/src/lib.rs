...
let weights = ebcandle::safetensors::load(weights_filename, &Device::Cpu).unwrap();
...
let weights = ebcandle::safetensors::load_buffer(&mmap[..], &Device::Cpu).unwrap();
...
// Convert safetensors Dtype to ebcandle DType
...
let mnist = ebcandle_datasets::vision::Dataset {
    train_images,
    train_labels,
    test_images,
    test_labels,
