...
use ebcandle_transformers::generation::{LogitsProcessor, Sampling};
use ebcandle_transformers::models::llama::Cache;

use anyhow::{bail, Error as E, Result};
use ebcandle::{DType, Device, IndexOp, Tensor};
use ebcandle_nn::VarBuilder;
use ebcandle_transformers::models::llava::config::{
    HFGenerationConfig, HFLLaVAConfig, HFPreProcessorConfig,
};
use ebcandle_transformers::models::llava::{config::LLaVAConfig, LLaVA};
use clap::Parser;
use constants::*;
...
struct Args {
    ...
    #[arg(long)]
    prompt: String,
    /// The seed to use when generating random samples. Copy from ebcandle llama. Not exist in python llava.
    #[arg(long, default_value_t = 299792458)]
    seed: u64,
}

//from https://github.com/huggingface/ebcandle/blob/main/ebcandle-examples/examples/clip/main.rs
...
fn main() -> Result<()> {
    ...
    let weight_filenames =
        ebcandle_examples::hub_load_safetensors(&api, "model.safetensors.index.json")?;
    ...
    //inference loop, based on https://github.com/huggingface/ebcandle/blob/main/ebcandle-examples/examples/llama/main.rs
    let mut tokenizer = ebcandle_examples::token_output_stream::TokenOutputStream::new(tokenizer);
    let mut index_pos = 0;
