...
use ebcandle::{DType, IndexOp, D};
use ebcandle_nn::{Module, VarBuilder};
use ebcandle_transformers::models::efficientnet::{EfficientNet, MBConvConfig};
...
pub fn main() -> anyhow::Result<()> {
    ...
    let image = ebcandle_examples::imagenet::load_image224(args.image)?.to_device(&device)?;
    println!("loaded image {image:?}");

    let model_file = match args.model {
        None => {
            let api = hf_hub::api::sync::Api::new()?;
            let api = api.model("lmz/ebcandle-efficientnet".into());
    ...
    let model = EfficientNet::new(vb, cfg, ebcandle_examples::imagenet::CLASS_COUNT as usize)?;
    println!("model built");
    let logits = model.forward(&image.unsqueeze(0)?)?;
    let prs = ebcandle_nn::ops::softmax(&logits, D::Minus1)?
        .i(0)?
        .to_vec1::<f32>()?;
    ...
    for &(category_idx, pr) in prs.iter().take(5) {
        println!(
            "{:24}: {:.2}%",
            ebcandle_examples::imagenet::CLASSES[category_idx],
            100. * pr
        );
    }
    Ok(())
