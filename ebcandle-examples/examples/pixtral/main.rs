...
use ebcandle_transformers::models::pixtral::{vision_model, Config, Model};

use ebcandle::{DType, Device, Module, Tensor};
...
fn main() -> Result<()> {
    ...
    let image = if args.image.ends_with(".safetensors") {
        match ebcandle::safetensors::load(&args.image, &device)?.remove("img") {
            None => anyhow::bail!("no img tensor in {}", args.image),
            Some(v) => v,
        }
    } else {
        ebcandle_examples::imagenet::load_image_with_std_mean(
            &args.image,
            1024,
