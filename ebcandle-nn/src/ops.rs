use ebcandle::{CpuStorage, DType, Layout, Module, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
///
/// ```rust
/// use ebcandle::{Tensor, Device, test_utils::to_vec2_round};
/// let a = Tensor::new(&[[0f32, 1., 0., 1.], [-2., 2., 3., -3.]], &Device::Cpu)?;
/// let a = ebcandle_nn::ops::softmax(&a, 1)?;
/// assert_eq!(
///     to_vec2_round(&a, 4)?,
///     &[
...
/// # Ok::<(), ebcandle::Error>(())
/// ```
pub fn softmax<D: ebcandle::shape::Dim>(xs: &Tensor, dim: D) -> Result<Tensor> {
...
pub fn log_softmax<D: ebcandle::shape::Dim>(xs: &Tensor, d: D) -> Result<Tensor> {
...
impl ebcandle::CustomOp1 for Sigmoid {
    fn name(&self) -> &'static str {
        "sigmoid"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        use ebcandle::backend::BackendStorage;

        fn fwd<T: num_traits::Float>(v: T) -> T {
            (v.neg().exp() + T::one()).recip()
        }

        // FIXME: using `ebcandle::map_dtype` causes compilation errors.
        let storage = match storage {
            CpuStorage::BF16(slice) => {
                CpuStorage::BF16(ebcandle::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F16(slice) => {
                CpuStorage::F16(ebcandle::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F32(slice) => {
                CpuStorage::F32(ebcandle::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F64(slice) => {
                CpuStorage::F64(ebcandle::cpu_backend::unary_map(slice, layout, fwd))
            }
            _ => Err(ebcandle::Error::UnsupportedDTypeForOp(
                storage.dtype(),
                self.name(),
            ))?,
        };
    ...
    fn cuda_fwd(
        &self,
        storage: &ebcandle::CudaStorage,
        layout: &Layout,
    ) -> Result<(ebcandle::CudaStorage, Shape)> {
        use ebcandle::backend::BackendStorage;
        use ebcandle::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig, ValidAsZeroBits,
        };
        use ebcandle::cuda_backend::SlicePtrOrNull;
        use ebcandle::cuda_backend::{kernel_name, kernels, Map1, WrapErr};
        use ebcandle::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        &self,
        storage: &ebcandle::MetalStorage,
        layout: &Layout,
    ) -> Result<(ebcandle::MetalStorage, Shape)> {
        use ebcandle::backend::BackendStorage;
        use ebcandle::MetalError;
        ...
        let src = ebcandle_metal_kernels::BufferOffset {
        ...
        match (el_count % 2, dtype, layout.is_contiguous()) {
            (0, DType::BF16 | DType::F16, true) => {
                use ebcandle_metal_kernels::unary::contiguous_tiled;
                ...
                ebcandle_metal_kernels::call_unary_contiguous_tiled(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
            ...
            (_, _, true) => {
                use ebcandle_metal_kernels::unary::contiguous;
                let kernel_name = match dtype {
                    DType::F16 => contiguous::sigmoid::HALF,
                    DType::F32 => contiguous::sigmoid::FLOAT,
                    DType::BF16 => contiguous::sigmoid::BFLOAT,
                    dtype => {
                        ebcandle::bail!("Metal contiguous unary sigmoid {dtype:?} not implemented")
                    }
                };
                ebcandle_metal_kernels::call_unary_contiguous(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
            ...
            (_, _, false) => {
                use ebcandle_metal_kernels::unary::strided;
                let kernel_name = match dtype {
                    DType::F16 => strided::sigmoid::HALF,
                    DType::F32 => strided::sigmoid::FLOAT,
                    DType::BF16 => strided::sigmoid::BFLOAT,
                    dtype => {
                        ebcandle::bail!("Metal strided unary sigmoid {dtype:?} not implemented")
                    }
                };
                let dst = ebcandle_metal_kernels::BufferOffset::zero_offset(&buffer);
                ebcandle_metal_kernels::call_unary_strided(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
        ...
        let new_storage = ebcandle::MetalStorage::new(buffer, device.clone(), el_count, dtype);
...
pub fn dropout(xs: &Tensor, drop_p: f32) -> Result<Tensor> {
    ...
    if !(0. ..1.).contains(&drop_p) {
        ebcandle::bail!("dropout probability has to be in [0, 1), got {drop_p}")
    }
...
impl ebcandle::ModuleT for Dropout {
...
impl ebcandle::CustomOp1 for SoftmaxLastDim {
    fn name(&self) -> &'static str {
        "softmax-last-dim"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        fn softmax<T: ebcandle::WithDType + num_traits::Float>(
            src: &[T],
            layout: &Layout,
        ) -> Result<(CpuStorage, Shape)> {
            let src = match layout.contiguous_offsets() {
                None => ebcandle::bail!("input has to be contiguous"),
            ...
            let storage = ebcandle::WithDType::to_cpu_storage_owned(dst);
        ...
        match storage {
            ...
            _ => ebcandle::bail!("unsupported dtype for softmax {:?}", storage),
        }
    }
    ...
    fn cuda_fwd(
        ...
        let dev = storage.device();
        let slice = S.map(&storage.slice, dev, layout)?;
        let dst = ebcandle::cuda_backend::CudaStorage {
            slice,
            device: dev.clone(),
        };
        Ok((dst, layout.shape().clone()))
    ...
    fn metal_fwd(
        ...
        let name = match storage.dtype() {
            ...
            dtype => ebcandle::bail!("softmax-last-dim is not implemented for {dtype:?}"),
        };

        let n = layout.stride().len();
        if !(layout.is_contiguous() && layout.stride()[n - 1] == 1) {
            ebcandle::bail!("Non contiguous softmax-last-dim is not implemented");
        }

        let last_dim = layout.dims()[layout.shape().rank() - 1];
        let elem_count = layout.shape().elem_count();
        let output = device.new_buffer(elem_count, storage.dtype(), "softmax")?;
        ebcandle_metal_kernels::call_last_softmax(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        .map_err(ebcandle::Error::wrap)?;
        let newstorage =
            ebcandle::MetalStorage::new(output, device.clone(), elem_count, storage.dtype());
...
impl ebcandle::CustomOp2 for RmsNorm {
    fn name(&self) -> &'static str {
        "rms-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let alpha = match alpha_layout.contiguous_offsets() {
                None => ebcandle::bail!("alpha has to be contiguous"),
        ...
        match (s1, s2) {
            ...
            _ => ebcandle::bail!("unsupported dtype for rmsnorm {:?}", s1.dtype()),
        }
    }
    ...
    fn cuda_fwd(
        &self,
        s1: &ebcandle::CudaStorage,
        l1: &Layout,
        s2: &ebcandle::CudaStorage,
        l2: &Layout,
    ) -> Result<(ebcandle::CudaStorage, Shape)> {
        use ebcandle::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig,
        };
        use ebcandle::cuda_backend::{kernel_name, kernels, Map2, WrapErr};
        use ebcandle::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        &self,
        s1: &ebcandle::MetalStorage,
        l1: &Layout,
        s2: &ebcandle::MetalStorage,
        l2: &Layout,
    ) -> Result<(ebcandle::MetalStorage, Shape)> {
        use ebcandle::backend::BackendStorage;
        ...
        let name = match (s1.dtype(), s2.dtype()) {
            ...
            (dt1, dt2) => ebcandle::bail!("rmsnorm is not implemented for {dt1:?} {dt2:?}"),
        };

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            ebcandle::bail!("Non contiguous rmsnorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "rmsnorm")?;
        ebcandle_metal_kernels::call_rms_norm(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        .map_err(ebcandle::Error::wrap)?;
        let newstorage = ebcandle::MetalStorage::new(output, device.clone(), elem_count, s1.dtype());
...
impl ebcandle::CustomOp3 for LayerNorm {
    fn name(&self) -> &'static str {
        "layer-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let beta = match beta_layout.contiguous_offsets() {
                None => ebcandle::bail!("beta has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        s2: &ebcandle::CudaStorage,
        l2: &Layout,
        s3: &ebcandle::CudaStorage,
        l3: &Layout,
    ) -> Result<(ebcandle::CudaStorage, Shape)> {
        use ebcandle::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig,
        };
        use ebcandle::cuda_backend::{kernel_name, kernels, Map3, WrapErr};
        use ebcandle::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype(), s3.dtype()) {
            ...
            (dt1, dt2, dt3) => {
                ebcandle::bail!("layernorm is not implemented for {dt1:?} {dt2:?} {dt3:?}")
            }
        };

        if !(l1.is_contiguous() && l2.is_contiguous() && l3.is_contiguous()) {
            ebcandle::bail!("Non contiguous layernorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "layernorm")?;
        ebcandle_metal_kernels::call_layer_norm(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
...
pub fn replication_pad2d(xs: &Tensor, pad: usize) -> Result<Tensor> {
    match pad {
        ...
        n => ebcandle::bail!("replication-pad with a size of {n} is not supported"),
    }
}
...
pub fn kvconcat(ltensor: &Tensor, rtensor: &Tensor, concat_dim: usize) -> Result<Tensor> {
    ...
    use ebcandle::cuda_backend::KVConcat;
...
impl ebcandle::CustomOp3 for Sdpa {
    fn name(&self) -> &'static str {
        "sdpa"
    }
    ...
    fn cpu_fwd(
        ...
        ebcandle::bail!("SDPA has no cpu impl")
    }

    #[cfg(feature = "metal")]
    fn metal_fwd(
        &self,
        q: &ebcandle::MetalStorage,
        q_l: &Layout,
        k: &ebcandle::MetalStorage,
        k_l: &Layout,
        v: &ebcandle::MetalStorage,
        v_l: &Layout,
    ) -> Result<(ebcandle::MetalStorage, Shape)> {
        use ebcandle::backend::BackendStorage;
        use ebcandle_metal_kernels::SdpaDType;
        ...
        if q_l.dims()[q_l.dims().len() - 1] != k_l.dims()[k_l.dims().len() - 1] {
            ebcandle::bail!("`q` and `k` last dims must match");
        }

        // k,v must have matching n kv heads
        if v_l.dims()[v_l.dims().len() - 3] != k_l.dims()[k_l.dims().len() - 3] {
            ebcandle::bail!("`k` and `v` head dims must match");
        }

        // n_heads % n_kv_heads == 0; n_heads >= 1, n_kv_heads >= 1.
        if q_l.dims()[q_l.dims().len() - 3] % k_l.dims()[k_l.dims().len() - 3] != 0 {
            ebcandle::bail!("query `n_heads` must be a multiple of `n_kv_heads`");
        }
        ...
        for t in [k.dtype(), v.dtype()] {
            if q.dtype() != t {
                ebcandle::bail!("all q, k, v dtypes must match.");
            }
        }
        ...
        if supports_sdpa_vector {
            ...
            ebcandle_metal_kernels::call_sdpa_full(
                q.device().device(),
                &command_buffer,
                q.device().kernels(),
                q_l.start_offset(),
            ...
            ebcandle::bail!("must be vector or full sdpa kernel");
        }

        let newstorage = ebcandle::MetalStorage::new(output, device.clone(), elem_count, q.dtype());
