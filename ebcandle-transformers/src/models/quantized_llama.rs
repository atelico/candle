...
use ebcandle_nn::{scaled_dot_product_attention, Embedding, Module};
...
impl QMatMul {
    fn from_qtensor(qtensor: QTensor) -> Result<Self> {
        let inner = ebcandle::quantized::QMatMul::from_qtensor(qtensor)?;
...
impl Module for Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        self.feed_forward_w2
            .forward(&(ebcandle_nn::ops::silu(&w1)? * w3)?)
    }
}
...
impl LayerWeights {
    fn apply_rotary_emb(&self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        ebcandle_nn::rotary_emb::rope_i(&x.contiguous()?, &cos, &sin)
    }
...
impl ModelWeights {
    ...
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let md_get = |s: &str| match ct.metadata.get(s) {
            None => ebcandle::bail!("cannot find {s} in metadata"),
            Some(v) => Ok(v),
        };
