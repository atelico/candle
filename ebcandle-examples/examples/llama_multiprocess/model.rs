...
use ebcandle::{CpuStorage, CustomOp1, DType, Device, IndexOp, Layout, Result, Shape, Tensor, D};
use ebcandle_nn::var_builder::ShardedVarBuilder as VarBuilder;
use ebcandle_nn::{Embedding, Linear, Module, RmsNorm};
...
pub type Config = ebcandle_transformers::models::llama::LlamaConfig;
...
impl CustomOp1 for AllReduce {
    ...
    fn cpu_fwd(&self, _s: &CpuStorage, _l: &Layout) -> Result<(CpuStorage, Shape)> {
        ebcandle::bail!("AllReduce is never used on cpu")
    }

    #[cfg(feature = "cuda")]
    fn cuda_fwd(
        &self,
        s: &ebcandle::CudaStorage,
        l: &Layout,
        ...
        let dst = match s.dtype() {
            DType::BF16 => {
                ...
                let s = match l.contiguous_offsets() {
                    ...
                    Some(_) | None => ebcandle::bail!("input has to be contiguous"),
                };
                let mut dst = unsafe { dev.alloc::<bf16>(elem_count) }.w()?;
                self.comm
                    .all_reduce(s, &mut dst, &ReduceOp::Sum)
                    .map_err(ebcandle::Error::debug)?;
                ebcandle::CudaStorage::wrap_cuda_slice(dst, dev)
            }
            DType::F16 => {
            ...
            dtype => ebcandle::bail!("unsupported dtype {dtype:?}"),
        };
...
fn shard(dim: usize, rank: usize, world_size: usize) -> ebcandle_nn::var_builder::Shard {
    ebcandle_nn::var_builder::Shard {
        dim,
        rank,
        world_size,
    }
...
impl CausalSelfAttention {
    fn apply_rotary_emb(&self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        ebcandle_nn::rotary_emb::rope(x, &cos, &sin)
    }
    ...
    fn forward(&self, x: &Tensor, index_pos: usize, block_idx: usize) -> Result<Tensor> {
        ...
        let y = ebcandle_flash_attn::flash_attn(&q, &k, &v, softmax_scale, seq_len > 1)?
    ...
    fn repeat_kv(&self, x: Tensor) -> Result<Tensor> {
        ...
        ebcandle_transformers::utils::repeat_kv(x, n_rep)
    }
