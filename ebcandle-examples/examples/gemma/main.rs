...
use ebcandle_transformers::models::gemma::{Config as Config1, Model as Model1};
use ebcandle_transformers::models::gemma2::{Config as Config2, Model as Model2};
...
impl Model {
    fn forward(&mut self, input_ids: &Tensor, pos: usize) -> ebcandle::Result<Tensor> {
        match self {
...
fn main() -> Result<()> {
    ...
    let filenames = match args.weight_files {
        Some(files) => files
            ...
            .collect::<Vec<_>>(),
        None => ebcandle_examples::hub_load_safetensors(&repo, "model.safetensors.index.json")?,
    };
