...
use ebcandle::{DType, Device, Module, Result, Tensor, D};
use ebcandle_nn::{linear_b as linear, Activation, Linear, VarBuilder};
...
impl Config {
    fn hidden_act(&self) -> Result<Activation> {
        match (self.hidden_act, self.hidden_activation) {
            ...
            (Some(_), Some(_)) => ebcandle::bail!("both hidden_act and hidden_activation are set"),
            (None, None) => ebcandle::bail!("none of hidden_act and hidden_activation are set"),
        }
    }
}
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(
        ...
        let q_embed = ebcandle_nn::rotary_emb::rope(&q.contiguous()?, &cos, &sin)?;
        let k_embed = ebcandle_nn::rotary_emb::rope(&k.contiguous()?, &cos, &sin)?;
        Ok((q_embed, k_embed))
    }
}
...
#[derive(Debug, Clone)]
pub struct Model {
    embed_tokens: ebcandle_nn::Embedding,
...
impl Model {
    pub fn new(use_flash_attn: bool, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_tokens =
            ebcandle_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb_m.pp("embed_tokens"))?;
    ...
    pub fn embed_tokens(&self) -> &ebcandle_nn::Embedding {
        &self.embed_tokens
    }
