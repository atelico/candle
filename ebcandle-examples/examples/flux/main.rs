...
use ebcandle_transformers::models::{clip, flux, t5};

use anyhow::{Error as E, Result};
use ebcandle::{IndexOp, Module, Tensor};
...
fn run(args: Args) -> Result<()> {
    ...
    let device = ebcandle_examples::device(cpu)?;
    ...
    let img = match decode_only {
            ...
            let img = {
                ...
                let state = if quantized {
                    flux::sampling::State::new(
                        &t5_emb.to_dtype(ebcandle::DType::F32)?,
                        &clip_emb.to_dtype(ebcandle::DType::F32)?,
                        &img.to_dtype(ebcandle::DType::F32)?,
                    )?
                } else {
                ...
                if quantized {
                    let model_file = match model {
                        Model::Schnell => api
                            .repo(hf_hub::Repo::model("lmz/ebcandle-flux".to_string()))
        ...
        Some(file) => {
            let mut st = ebcandle::safetensors::load(file, &device)?;
    ...
    println!("img\n{img}");
    let img = ((img.clamp(-1f32, 1f32)? + 1.0)? * 127.5)?.to_dtype(ebcandle::DType::U8)?;
    ebcandle_examples::save_image(&img.i(0)?, "out.jpg")?;
    Ok(())
}
...
fn main() -> Result<()> {
    let args = Args::parse();
    #[cfg(feature = "cuda")]
    ebcandle::quantized::cuda::set_force_dmmv(args.use_dmmv);
    run(args)
}
