...
use ebcandle_transformers::object_detection::{non_maximum_suppression, Bbox, KeyPoint};
...
pub fn report_detect(
    ...
    let font = ab_glyph::FontRef::try_from_slice(&font).map_err(ebcandle::Error::wrap)?;
    for (class_index, bboxes_for_class) in bboxes.iter().enumerate() {
        for b in bboxes_for_class.iter() {
            println!(
                "{}: {:?}",
                ebcandle_examples::coco_classes::NAMES[class_index],
                b
            );
...
pub fn report_pose(
    ...
    if pred_size != 17 * 3 + 4 + 1 {
        ebcandle::bail!("unexpected pred-size {pred_size}");
    }
    let mut bboxes = vec![];
    // Extract the bounding boxes for which confidence is above the threshold.
    for index in 0..npreds {
    ...
    fn model(&self) -> anyhow::Result<std::path::PathBuf> {
        let path = match &self.model {
            ...
            None => {
                ...
                let api = api.model("lmz/ebcandle-yolo-v8".to_string());
